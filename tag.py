...
note : works only on a xterm compatible terminal
"""
...
try:
    ...
    from mutagenx.id3 import TIT2, TALB, TPE1, WOAF, TCOM, TCON, APIC, TRCK
except ImportError:
...
def searchItunes(args, query):
    ...
    tag['album'] = trackSelected['collectionName'] or 'Single'                  #itunes sometimes doesn't return album
...
def main(argv):
    ...
    parser.add_argument('-b','--hi-res', help='use higher resolution cover arts',action='store_true')
    ...
    for track in args.songs:
        ...
        if args.i:
            data = searchItunes(args, keywords)
        else:
            data = searchSpotify(args, keywords)
        try:
            tagmp3(data, audio)
...
def tagmp3(tagdata, audio):
    ...
    try:
        audio['TRCK'] = TRCK(encoding=3, text=tagdata['trackNumber'])
    except Exception:
        pass
    if 'TDRC' in tagdata.keys():
        audio['TDRC'] = TDRC(encoding=3, text=tagdata['releaseDate'][:10])
